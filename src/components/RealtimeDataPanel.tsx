...
export function RealtimeDataPanel({ data }: RealtimeDataPanelProps) {
  const parameters = [
    ...
    },
    {
      ...
      unit: "KVARh",
      description: "Energía Reactiva Retraso",
      icon: Activity,
      color: "text-chart-4"
    },
    {
      id: "kvarh_lead",
      label: "KVARh (Adelanto)",
      value: data.kvarh_lead,
      unit: "KVARh",
      description: "Energía Reactiva Adelanto",
      icon: Activity,
      color: "text-chart-5"
    }
  ];
  ...
  const getPowerFactorStatus = (pf: number) => {
    if (pf >= 0.95) return { status: "Excelente", variant: "default" as const, color: "bg-green-500" };
    if (pf >= 0.85) return { status: "Bueno", variant: "secondary" as const, color: "bg-yellow-500" };
    return { status: "Pobre", variant: "destructive" as const, color: "bg-red-500" };
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Lecturas de parámetros en vivo - Última actualización: {data.timestamp}
        </CardDescription>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-4 bg-accent/50 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="font-medium">Estado del Factor de Potencia</h4>
              <p className="text-sm text-muted-foreground">Indicador de eficiencia del sistema</p>
            </div>
        ...
        <div className="space-y-3">
          <h4 className="font-medium text-sm text-muted-foreground">PARÁMETROS DE ENERGÍA</h4>
